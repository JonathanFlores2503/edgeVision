...
class EdgeApp:
    def __init__(self, cfg: EdgeConfig, mock: bool = False, config_path: str | None = None):
        ...
        # Biblioteca de vídeos subidos desde el dashboard (cámaras `file://…`).
        # Se ancla junto al config.yaml, igual que settings.json, para que la web y
        # la captura miren el mismo directorio venga de donde venga el CWD.
        video_file.configure(self._anchored(cfg.storage.videos_dir),
                             max_mb=cfg.storage.max_video_mb)
        ...
        # El modelo puede pedir otra resolución de captura (duck-typing, como
        # supports_zones/supports_counts). Hace falta porque la captura reduce cada
        # frame a `capture.proc_short_side` (360 px) ANTES de que el modelo lo vea: con
        # una cámara 1080p eso convierte un rostro de 40 px en uno de 13 y el
        # reconocimiento facial se queda a cero, sin que nada falle. Solo el modelo que
        # lo necesita paga la RAM extra; los demás siguen con el reescalado barato.
        pista = self._capture_hints(cfg)
        self.manager = CameraManager(
            static_cams=self._configured_cameras(),
            model_name=cfg.model.name,
            ring_seconds=pista["ring_seconds"],
            on_clip=(self._ignore_clip if self._is_stream else self.worker.submit),
            on_frame=self._make_frame_tap(),
            capture_cfg=pista["capture_cfg"],
    ...
    def _capture_hints(self, cfg) -> dict:
        """Resuelve resolución de captura y ring buffer, dejando que el modelo opine.

        El modelo devuelve en `capture_hints()` lo que necesita
        (`{"short_side": 0, "ring_seconds": 4.0}`); `short_side=0` es full-res. El ring
        se recorta a propósito cuando se sube la resolución: son
        `ring_seconds x fps x ancho x alto x 3` bytes por cámara en RAM, y a 1080p los
        12 s por defecto son ~1.1 GB en una placa de 7.4 GB. El mínimo útil es
        `events.pre_roll_s`, que es lo que el clip necesita de antes del disparo.
        """
        capture_cfg = cfg.capture
        ring_s = cfg.storage.ring_buffer_s
        fn = getattr(self.model, "capture_hints", None)
        if not callable(fn):
            return {"capture_cfg": capture_cfg, "ring_seconds": ring_s}
        try:
            h = fn() or {}
        except Exception:  # noqa: BLE001 — un modelo raro no cambia la captura
            log.exception("El modelo activo falló al declarar capture_hints(); se usa "
                          "la configuración del archivo.")
            return {"capture_cfg": capture_cfg, "ring_seconds": ring_s}
        short = h.get("short_side")
        if short is not None and int(short) != capture_cfg.proc_short_side:
            capture_cfg = capture_cfg.model_copy(
                update={"proc_short_side": int(short)})
            log.info("El modelo pide captura %s (config: %s px de lado corto).",
                     (f"a {int(short)} px de lado corto" if int(short)
                      else "a resolución NATIVA, sin reescalar"),
                     cfg.capture.proc_short_side)
        nuevo_ring = h.get("ring_seconds")
        if nuevo_ring is not None:
            minimo = float(cfg.events.pre_roll_s)
            nuevo_ring = max(float(nuevo_ring), minimo)
            if nuevo_ring != ring_s:
                log.info("Ring buffer a %.1f s por petición del modelo (config: %.1f s; "
                         "mínimo por pre_roll_s: %.1f s).", nuevo_ring, ring_s, minimo)
                ring_s = nuevo_ring
        return {"capture_cfg": capture_cfg, "ring_seconds": ring_s}
    ...
    def _apply_model_override(self) -> None:
        """Aplica la selección de pesos (detector/clasificador) hecha desde admin."""
    ...
    def _anchored(self, path: str) -> str:
        """Resuelve una ruta relativa del config.yaml JUNTO al propio config.yaml,
        no al CWD (así el nodo lee/escribe lo mismo se lance desde donde se lance)."""
        p = Path(path).expanduser()
        if p.is_absolute():
            return str(p)
        base = (Path(self._config_path).resolve().parent
                if self._config_path else Path.cwd())
    ...
    def _configured_cameras(self) -> "list[CameraCfg]":
        """Fuentes a capturar (RTSP, USB o vídeo subido), tomadas de la fuente
        editable de la interfaz (`data/settings.json`) para que lo que el usuario
        ve/edita en la plataforma sea lo que de verdad se captura. Cae a
        `config.yaml` si no hay monitor."""
        if self.settings is None:
    ...
    #: Por debajo de esta RAM disponible el nodo avisa en el log (ver _stats_loop).
    LOW_MEM_WARN_MB = 1200.0
        ...
        while not self._stop.wait(2.0):
            ...
            if psutil is not None:
                try:
                    ...
                    # Aviso de RAM baja. La placa tiene 7.4 GB compartidos con el
                    # escritorio y el modelo se come 4.5: al quedarse sin memoria el
                    # dashboard dejaba de responder sin una línea en el log y parecía
                    # un fallo del login. Que se vea, y con el dato a mano.
                    avail_mb = vm.available / 1e6
                    if avail_mb < self.LOW_MEM_WARN_MB and (time.time() - last_warn) > 30.0:
                        last_warn = time.time()
                        log.warning("RAM baja: %.0f MB disponibles de %.0f MB "
                                    "(este proceso usa %.0f MB). El dashboard puede "
                                    "dejar de aceptar conexiones; baja el número de "
                                    "cámaras o arranca la placa sin escritorio.",
                                    avail_mb, vm.total / 1e6,
    ...
    # ── Ciclo de vida ────────────────────────────────────────────────────────
    def run(self):
        log.info("Iniciando nodo edge '%s' con %d fuente(s) configurada(s) "
                 "(RTSP / vídeo subido)%s.",
        ...
        # Volcado de pilas a demanda: `kill -USR1 <pid>` imprime la pila de todos los
        # hilos. faulthandler escribe desde el propio manejador de señal, sin pedir el
        # GIL, así que es lo único que habla cuando el proceso se congela porque un
        # hilo bloqueado en C (CUDA, cv2) no lo suelta.
        faulthandler.enable()
        if hasattr(signal, "SIGUSR1"):
            faulthandler.register(signal.SIGUSR1, all_threads=True, chain=False)
            log.info("Diagnóstico de cuelgues: kill -USR1 %d volcará la pila de los hilos.",
                     os.getpid())
        signal.signal(signal.SIGINT, self._on_signal)
        signal.signal(signal.SIGTERM, self._on_signal)
        # Se espera EN BUCLE y con timeout, nunca `self._stop.wait()` a secas. El kernel
        # entrega la señal a cualquier hilo del proceso; si no le toca al principal, su
        # espera sin timeout no se interrumpe y el manejador de Python —que solo corre en
        # el hilo principal— no llega a ejecutarse jamás. Con ~40 hilos (captura, torch,
        # ffmpeg, CLIP) casi nunca le toca: eso era el nodo que ignoraba el SIGTERM y no
        # cerraba con Ctrl+C. Despertando cada 0.5 s el hilo principal ejecuta bytecode y
        # ahí sí atiende la señal.
        while not self._stop.wait(0.5):
            pass
        self.shutdown()

    def _on_signal(self, signum, _frame):
        """Apagado ordenado a la 1ª señal; salida a la fuerza a la 2ª.

        Sin la segunda rama, un hilo que no cerraba dejaba el nodo inmortal: Ctrl+C
        no hacía nada y había que ir a otra terminal a por el `kill -9`.
        """
        if self._stop.is_set():
            log.warning("Segunda señal (%d): salida inmediata. Hilos vivos:", signum)
            faulthandler.dump_traceback()
            os._exit(130)
        log.info("Señal %d recibida: apagando el nodo...", signum)
        self._stop.set()

    #: Segundos que se le dan al apagado antes de volcar las pilas y salir a la fuerza.
    SHUTDOWN_TIMEOUT_S = 25.0

    def shutdown(self):
        log.info("Apagando nodo edge...")
        # Reloj de la muerte. Lo lleva un hilo en C de faulthandler, que no necesita el
        # GIL: si algún hilo no cierra —o el intérprete entero se congela— vuelca la
        # pila de todos los hilos y mata el proceso. Antes el nodo se quedaba colgado
        # ignorando el SIGTERM y había que buscarlo con `pkill -9`.
        faulthandler.dump_traceback_later(self.SHUTDOWN_TIMEOUT_S, exit=True)
        # Cada pieza se detiene por separado: si una falla, las demás siguen cerrando
        # (antes una excepción a mitad dejaba la base de datos y el dashboard abiertos).
        steps = [
            ("captura", self.manager.stop),
            ("worker de inferencia", self.worker.stop if self.worker else None),
            ("modelo streaming", self.model.stop if self._is_stream else None),
            ("dispatcher", self.dispatcher.stop),
            ("OTA", self.ota.stop if self.ota else None),
            ("MQTT", self.mqtt.stop if self.mqtt else None),
            ("GPS", self.gps.stop if self.gps else None),
            ("dashboard", self.monitor_server.stop if self.monitor_server else None),
            ("base de datos", self.store.close),
        ]
        for label, stop_fn in steps:
            if stop_fn is None:
                continue
            try:
                stop_fn()
            except Exception:  # noqa: BLE001 — un fallo no puede abortar el resto
                log.exception("Error deteniendo %s; se continúa con el apagado.", label)
...
def _free_port_from_stale_node(port: int) -> None:
    ...
    victims: set[int] = set()
    have_ss = False
    try:  # 1) por dueños del puerto (ss) — el criterio preciso
        ...
        pass
    # 2) Respaldo por nombre SOLO si `ss` no está disponible. Es indiscriminado:
    # empareja cualquier `edge.main`, incluido un nodo legítimo escuchando en OTRO
    # puerto (p. ej. dos nodos conviviendo). Con `ss` presente, su lista de dueños
    # del puerto ya es la respuesta correcta, aunque venga vacía.
    if not have_ss:
        try:
            ...
            pass
    victims.discard(me)
    # NUNCA matar nuestro propio árbol de procesos. Al lanzar con
    # `uv run python -m edge.main`, el padre `uv run` lleva "edge.main" en su línea
    # de comandos, así que `pgrep` lo empareja: matarlo nos tumba a nosotros mismos
    # antes de arrancar (el shell vuelve al prompt y el puerto sigue ocupado).
    # Todo el job del shell comparte grupo de procesos, así que basta con excluirlo.
    try:
    ...
    for pid in victims:
        try:
            ...
            log.info("[--force] Nodo previo (pid %d): SIGTERM para liberar el puerto %d.", pid, port)
        except ProcessLookupError:
            pass
        except Exception:  # noqa: BLE001
            log.warning("[--force] No pude señalizar el pid %d.", pid)
    if not victims:
        return
    # El apagado ordenado puede colgarse (hilos de captura, modelos cargando) y
    # entonces el puerto no se libera nunca. Se espera, y al que no suelte se le
    # manda SIGKILL: con --force el usuario ya pidió explícitamente que arranque.
    ...
    for pid in victims:
        try:
            ...
            log.warning("[--force] El pid %d ignoró el SIGTERM: enviado SIGKILL.", pid)
        except ProcessLookupError:
            pass
        except Exception:  # noqa: BLE001
            log.warning("[--force] No pude matar el pid %d.", pid)
    time.sleep(1.0)  # dar un momento a que el SO libere el socket


def _port_free(port: int) -> bool:
    """True si `port` ya se puede escuchar (el nodo previo lo soltó)."""
    import socket
