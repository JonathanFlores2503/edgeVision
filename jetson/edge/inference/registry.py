...
Dos familias (ver `base.py`):
  • VAD (built-in): el modelo configurado en `config.yaml` (torch) más el scorer
    `mock`. La selección de pesos detector/clasificador del VAD se maneja aparte
...
# ── Modelos ofrecidos en el panel (filtro de depuración) ─────────────────────
# Mientras se depura el contador, el panel de admin solo debe ofrecer ese modelo.
# Tener el VAD y el mock a la vista invita a cambiarlos sin querer y a perseguir
# después fallos del modelo equivocado — y el VAD, además, triplica la RAM del
# proceso (≈5.5 GB frente a ≈1.6 GB) en una placa de 7.4 GB.
#
# El filtro afecta al LISTADO y al modelo por defecto, no a `_all_specs`: una
# selección ya guardada sigue resolviéndose, para que activar el filtro no deje
# el nodo sin el modelo que estaba usando.
#
#     ONLY_MODELS = set()               -> se ofrecen todos (comportamiento normal)
#     ONLY_MODELS = {"contador_flujo"}  -> solo el contador de personas
#
# Ahora mismo se ofrecen los tres modelos heurísticos (contador de personas,
# detector de peleas y reconocimiento facial), que son los que corren en la placa.
# `faces` faltaba aquí desde que se integró: el nodo lo arrancaba igual —una
# selección guardada se resuelve por `_all_specs`, que no pasa por este filtro— pero
# el panel no lo listaba, así que no había forma de elegirlo desde el dashboard.
# El VAD `x3d_l` sigue fuera de la lista por RAM: son ~5.5 GB de proceso frente a
# ~1.6 GB, y en 7.4 GB no conviene ofrecerlo por descuido junto a los demás.
ONLY_MODELS = {"contador_flujo", "fight_detector", "faces"}


def _offered(specs) -> List[ModelSpec]:
    """Aplica `ONLY_MODELS` a una lista de specs."""
    if not ONLY_MODELS:
        return list(specs)
    return [s for s in specs if s.key in ONLY_MODELS]
...
def list_models(model_cfg=None) -> Dict[str, List[dict]]:
    ...
    vad = _offered(_vad_specs(model_cfg))
    heur = _offered(discover_heuristics().values())
    if not vad and not heur:
        # `ONLY_MODELS` no casó con ninguna clave (typo, plugin renombrado). Un
        # panel sin ningún modelo seleccionable es peor que uno con todos.
        log.warning("ONLY_MODELS=%s no casa con ningún modelo; se ofrecen todos.",
                    sorted(ONLY_MODELS))
        vad, heur = _vad_specs(model_cfg), list(discover_heuristics().values())
    return {
        FAMILY_VAD: [s.to_public() for s in vad],
        FAMILY_HEURISTIC: [s.to_public() for s in heur],
    }
...
def _default_spec(model_cfg=None) -> ModelSpec:
    """Modelo a usar cuando no hay selección guardada (o no es válida).

    Sin filtro, el de siempre: el VAD configurado. Con `ONLY_MODELS` activo, el
    primero de los ofrecidos — si no, bastaría con borrar `settings.json` para
    que el nodo arrancara el VAD que precisamente queremos tener apartado."""
    vad_default = _vad_specs(model_cfg)[0]
    if not ONLY_MODELS:
        return vad_default
    offered = _offered(_vad_specs(model_cfg)) + _offered(discover_heuristics().values())
    return offered[0] if offered else vad_default
...
def resolve(selection: Optional[dict], model_cfg=None) -> ModelSpec:
    ...
    Si no hay selección válida, cae al modelo por defecto (ver `_default_spec`)."""
    vad_default = _default_spec(model_cfg)
    if not selection:
        return vad_default
