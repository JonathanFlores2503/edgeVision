"""Captura por cámara: lee frames, alimenta el ring buffer y forma clips.
...
Tres tipos de fuente, todas equivalentes aguas abajo:

  • RTSP/HTTP  — cámara de red (`rtsp://…`), abierta por FFmpeg.
  • USB local  — `/dev/video0` o `0`, abierta por V4L2.
  • Archivo    — vídeo subido desde el dashboard (`file://demo.mp4`), reproducido
                 en bucle y a su velocidad real (ver `video_file.py`).
"""
...
class CameraCapture:
    def __init__(
        ...
        # Fuente de archivo (vídeo subido): se reproduce en bucle y a su ritmo
        # nativo, para que se comporte como una cámara y no como un volcado de
        # disco a toda velocidad.
        self._is_file = video_file.is_video_source(cam.url)
        self._loop_file = bool(getattr(cam, "loop", True))
        self._frame_interval = 0.0   # s entre frames del archivo (0 = sin cadencia)
        self._next_frame_t = 0.0     # monotonic del próximo frame (0 = sin fijar)
    ...
    def _open(self) -> "cv2.VideoCapture | None":
        ...
        if self._is_file:
            cap = self._open_file(url)
        elif is_network_source(url):
            cap = self._open_network(url)
        else:
            cap = self._open_local(url)
        if cap is None:
            return None
    ...
    def _open_file(self, url: str) -> "cv2.VideoCapture | None":
        # Vídeo subido desde el dashboard: `file://demo.mp4` o una ruta suelta.
        path = video_file.resolve(url)
        if path is None:
            log.warning("[%s] No encuentro el vídeo '%s' (biblioteca: %s).",
                        self.cam.id, url, video_file.videos_dir())
            return None
        cap = cv2.VideoCapture(str(path), cv2.CAP_FFMPEG)
        if not cap.isOpened():
            log.warning("[%s] No se pudo abrir el vídeo %s", self.cam.id, path)
            cap.release()
            return None
        # Cadencia de reproducción: el FPS del contenedor. Si viene absurdo o no
        # viene (contenedores raros), asumimos 25.
        fps = float(cap.get(cv2.CAP_PROP_FPS) or 0.0)
        if not (1.0 <= fps <= 120.0):
            fps = 25.0
        self._frame_interval = 1.0 / fps
        self._next_frame_t = 0.0
        log.info("[%s] Vídeo abierto: %s (%.1f fps%s).", self.cam.id, path.name, fps,
                 ", en bucle" if self._loop_file else "")
        return cap

    def _pace_file(self) -> None:
        """Duerme lo que toque para reproducir el archivo a su velocidad real.

        Sin esto, OpenCV lee del disco tan rápido como puede: una cámara "de
        vídeo" quemaría la CPU y metería minutos de grabación en la cola de
        inferencia en segundos, con lo que los eventos saldrían con marcas de
        tiempo sin ningún parecido con la realidad."""
        ...
        if delay > 0:
            self._stop.wait(delay)   # `wait` y no `sleep`: el apagado es inmediato
        elif delay < -1.0:
            self._next_frame_t = now  # nos quedamos atrás (placa cargada): re-sincroniza

    def _rewind(self, cap: "cv2.VideoCapture") -> bool:
        """Vuelve al primer frame del archivo. False si el contenedor no admite
        seek (entonces se reabre desde cero)."""
        try:
    ...
    def _run(self):
        ...
        empty_loops = 0  # vueltas de bucle (archivo) sin haber leído ni un frame
        ...
        while not self._stop.is_set():
            ...
            if not ok and self._is_file:
                if not self._loop_file:
                    log.info("[%s] Vídeo terminado.", self.cam.id)
                    self._status("terminado")
                    break
                # Fin del archivo: rebobinar es lo normal, no una caída. Si el
                # contenedor no admite seek, se reabre desde cero.
                ...
                empty_loops += 1
                if empty_loops >= 2:
                    # Dos vueltas sin un solo frame: el archivo está vacío o roto.
                    # Esperamos antes de reintentar para no girar en vacío.
                    log.warning("[%s] El vídeo no entrega frames; reintento en %.0fs.",
                                self.cam.id, backoff)
                    self._status("sin señal")
                    live = False
                    if cap is not None:
            ...
            if self._is_file:
                empty_loops = 0
                self._pace_file()   # a la velocidad real del vídeo, no a la del disco
            if not live:
                live = True
