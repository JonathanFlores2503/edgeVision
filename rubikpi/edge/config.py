...
class CaptureCfg(BaseModel):
    ...
    # saturar la cola en CPU (RUBIK Pi 3, ~2.6 s/clip). 0 = clips contiguos (Jetson
    # con GPU rápida). El vídeo en vivo y el ring buffer NO se pausan; solo se deja
    # un hueco temporal entre los clips que se mandan a inferir.
...
class ModelCfg(BaseModel):
    ...
    # Backend de inferencia: "torch" (Jetson/PC, default) | "onnx" (RUBIK Pi 3,
    # onnxruntime con QNN/Hexagon o CPU). Ver edge/inference/model_onnx.py.
    backend: str = "torch"
    onnx_dir: str = "./data/onnx"   # carpeta de los .onnx (solo si backend == "onnx")
